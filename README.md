...
| 接入 OpenAI、智谱或新增 MaaS Provider | [Provider 指南](docs/PROVIDERS.md) |
...
确定性 `ScriptedAgent` 用于教程和 CI；`MaaSAgent` 用统一决策协议连接不同 Provider。OpenAI Responses：
...
模型必须显式指定，不使用会随时间变化的默认值。实际 Agent、Provider 和 model 会写入 manifest，resume 必须保持一致。Provider 协议、安全边界与智谱套餐使用提示见 [docs/PROVIDERS.md](docs/PROVIDERS.md)。
...
| Agent | ScriptedAgent + MaaSAgent + OpenAI/智谱 Provider |
